  ...
  return async (req, res, next) => {
    ...
    if (!ratelimit) {
      if (process.env.NODE_ENV === 'development') {
        logger.warn('Redis rate limiting not configured, skipping in development');
        return next(req, res);
      } else {
        // In production, Redis should be configured
        logger.error('Redis rate limiting not configured in production');
...
export function createRateLimiter(options = {}) {
  ...
  if (!redis) {
    logger.warn('Redis not configured, rate limiter will not work');
    return null;
  }
...
export function withPresetRateLimit(type = 'api') {
  return async (req, res, next) => {
    ...
    if (!limiter) {
      if (process.env.NODE_ENV === 'development') {
        logger.warn(`Rate limiter '${type}' not configured, skipping in development`);
        return next(req, res);
      } else {
        logger.error(`Rate limiter '${type}' not configured in production`);
        return sendRateLimitError(res);
      }
    }
